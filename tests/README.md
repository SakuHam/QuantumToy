...
| `test_spatial_effect_measurement.py` | Full spatial POVM positivity and completeness; complex operator coherences and global-phase invariance; complete no-click laws; exact `lambda=0` null and zero width information; injected one- and two-parameter recovery; width–coupling Fisher degeneracy; complementary detector design and independent-law combination at fixed total shots; locked real-potential double slit with unitary split-step evolution and interference response; visualization-frame consistency; temporal quadrature, propagation-step, box, grid, geometry-edge, and horizon checks. |
