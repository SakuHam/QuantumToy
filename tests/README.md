...
| `test_record_sensitivity.py` | Shared operational-threshold grid, monotonic threshold behavior, unresolved cases, event-clock time-sampling convergence, and fixed-alpha versus calibration-profiled width predictions across shared conventions. |
...
| `test_spatial_effect_measurement.py` | Full spatial POVM positivity and completeness; complex operator coherences and global-phase invariance; complete no-click laws; exact `lambda=0` null and zero width information; injected one- and two-parameter recovery; width–coupling Fisher degeneracy; complementary detector design over position, gate width, and readout time with both-error improvement at fixed total shots; locked real-potential double slit with unitary split-step evolution and interference response; visualization-frame consistency; temporal quadrature, propagation-step, box, grid, geometry-edge, and horizon checks. |
| `test_spatial_detector_inference.py` | Column-stochastic efficiency/dark-count/bin-blur detector channel; normalized timing-jitter mixture; independent free-control calibration; minimax Fisher design across a declared parameter region; shared nuisance profiling over calibration and double-slit counts; parametric-bootstrap threshold for the nonregular null; independent multinomial false-positive, recovery, and joint-coverage smoke tests; seed-addressable replicate diagnostics; nested-grid comparison on unchanged records with matching common-point likelihoods; visibility of nonwinning optimizer failures and parameter boundaries. |
| `test_spatial_temporal_observation.py` | Fixed temporal outcome grid; normalized half-Gaussian weights; complete integrated, resolved-time, timestamp-blurred, and coherent Fourier-port laws; exact shared spatial marginals; sigma-independent lambda-zero null; positive local Fisher information. |
| `test_spatial_arrival_time.py` | Phenomenological finite-gate joint `p(y,t)` instrument; source-pulse and detector timestamp convolution; complete dark/no-click outcomes; exact response null; spatial and temporal marginals; and the timing-width degeneracy that requires independent calibration. |
| `test_trf_physical_scale.py` | Schrödinger nondimensionalization `T0=mL0^2/hbar`; equivalent slit- and velocity-based scales; quadratic length scaling; half-normal direct-delay moments; and the explicit `alpha` bridge required between record and spatial clocks. |
