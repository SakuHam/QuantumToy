...
# ====================================================
# 6.5) Worldline bias forwardiin
# ====================================================
USE_WORLDLINE_FORWARD_BIAS = True

# peak-valinta
WL_CHOOSE_SECOND_PEAK = True
WL_TOPK_PEAKS = 3
WL_MIN_PEAK_DIST_PX = 18

# milloin bias alkaa sallia haaran valinnan
v_est = k0x / m_mass
t_barrier_est = (barrier_center_x - x0) / v_est
t_gap_est = (screen_center_x - barrier_center_x) / v_est
...
def worldline_gain_field(xc, yc, gain_sigma_x, gain_sigma_y):
    dxg = X - xc
    dyg = Y - yc
    tube = np.exp(-0.5 * ((dxg / gain_sigma_x)**2 + (dyg / gain_sigma_y)**2))
    return tube
...
def apply_forward_worldline_bias_preserve_norm(
    psi,
    xc,
    yc,
    dt,
    ...
    """
    Bias tehdään reaalisena amplitude-gainina:
      psi -> psi * exp(dt * ramp * (gain*tube - damp*(1-tube)))
    Lopuksi palautetaan alkuperäinen normi, jotta CAP-häviö säilyy.
    """
    if ramp_scale <= 0:
        return psi
    ...
    tube = worldline_gain_field(xc, yc, sigma_x, sigma_y)
...
def maybe_initialize_or_update_worldline(psi_cur, t_now, wl_state):
    ...
    # barrierin jälkeen, ennen screeniä
    branch_zone = (
    ...
    if not wl_state["locked"]:
        ...
        print(f"[WL-FWD] init at t={t_now:.3f}, found {len(peaks)} peaks")
        for j, (val, iy, ix) in enumerate(peaks[:WL_PRINT_TOP_PEAKS]):
            print(f"  peak#{j+1}: val={val:.6e}, x={X_vis[iy, ix]:.3f}, y={Y_vis[iy, ix]:.3f}, iy={iy}, ix={ix}")

        if WL_CHOOSE_SECOND_PEAK and len(peaks) >= 2:
            val, iy, ix = peaks[1]
            rank = 2
        else:
            val, iy, ix = peaks[0]
        ...
        wl_state["seed_rank"] = rank
        wl_state["ref_amp"] = max(float(val), 1e-30)
        wl_state["iy_vis"] = int(iy)
        wl_state["ix_vis"] = int(ix)
        wl_state["x"] = float(X_vis[iy, ix])
        wl_state["y"] = float(Y_vis[iy, ix])
        ...
        print(
            f"[WL-FWD] chosen peak#{rank}: "
            f"x={wl_state['x']:.3f}, y={wl_state['y']:.3f}, val={val:.6e}"
        )
        return wl_state

    # jo lukittu: trackaa lokaali huippu lähellä vanhaa
    iy0 = wl_state["iy_vis"]
    ix0 = wl_state["ix_vis"]
    val, iy_new, ix_new = extract_local_peak(prob_vis, iy0, ix0, radius_px=WL_TRACK_RADIUS_PX)

    if val >= WL_MIN_LOCAL_REL * wl_state["ref_amp"]:
        wl_state["iy_vis"] = int(iy_new)
        wl_state["ix_vis"] = int(ix_new)
        wl_state["x"] = float(X_vis[iy_new, ix_new])
        wl_state["y"] = float(Y_vis[iy_new, ix_new])
...
# ====================================================
# 7) Forward: ψ(t) + continuous measurement + worldline bias
# ====================================================
frames_psi = []
times      = []
norms_psi  = []
...
if USE_WORLDLINE_FORWARD_BIAS:
    print("  - worldline forward bias ON")
...
for n in range(n_steps + 1):
    ...
    if n % save_every == 0:
        ...
        if (len(frames_psi) % 20) == 0:
            lock_txt = "locked" if wl_state["locked"] else "searching"
    ...
    if n < n_steps:
        ...
        # update worldline tracker
        wl_state = maybe_initialize_or_update_worldline(psi_cur, t_now, wl_state)

        # apply worldline bias to forward dynamics
        if wl_state["locked"]:
            ramp_scale = ramp01(t_now, WL_ENABLE_TIME, WL_RAMP_TIME)
            psi_cur = apply_forward_worldline_bias_preserve_norm(
                psi_cur,
                wl_state["x"],
                wl_state["y"],
                dt=dt,
...
def update(i):
    ...
    update_worldline_overlay(i)
    title.set_text(rf"ρ(t): σT={sigma_current[0]:.3f}, t={times[i]:.3f}, norm≈{norms_psi[i]:.4f}")
    return (im, wl_line, wl_dot)

ani = FuncAnimation(fig, update, frames=Nt, interval=40, blit=False)

ani.save("worldline_advanced.mp4", writer="ffmpeg", fps=25, dpi=150)
