...
class SingleSlitBarrier:
    """
    Single-slit barrier with selectable edge model.
    ...
    ----------
    center_x:
        Barrier center position in x.

    thickness:
        Geometric barrier thickness.

    slit_half_height:
        Half-height of the slit around y=0.

    V_barrier:
        Barrier potential height.

    barrier_smooth:
        Smooth width used for edge_mode="smooth".

    sharp_smooth_width:
        Smooth width used for edge_mode="sharp_smooth".

    edge_mode:
        Which edge model to use.
    """
