...
class ThickFrontOptimizedTheory(SchrodingerTheory):
    ...
    This is cheaper than doing an FFT-based Gaussian coherence filter every step.
    """
    ...
    def _coherence_alignment_score(self, psi: np.ndarray):
        """
        Returns:
            align_real : in roughly [-1, 1], phase alignment with neighborhood
            rho        : local density
        ...
        if self.front_density_weighted:
            ...
            # This tends to make coherent higher-density branches win more often.
    ...
    def _front_sharpen(self, psi: np.ndarray, dt: float) -> np.ndarray:
        """
        Apply local thick-front sharpening.
        ...
        # Center score so that weakly aligned / misaligned regions are not boosted equally.
        # Positive alignment -> amplification, negative -> damping.
        gain = (
            self.front_strength * np.maximum(align_real, 0.0)
            - self.front_misaligned_damp * np.maximum(-align_real, 0.0)
        )

        # Mild density weighting to help coherent branches sharpen more naturally.
        ...
        psi_new = psi * np.exp(gain)

        # Optional tiny phase pull toward neighborhood coherent phase.
        # Keep this small. Too large will over-regularize interference structure.
        ...
        return psi_new.astype(np.complex128)
    ...
    def step_forward(self, state: np.ndarray, dt: float) -> TheoryStepResult:
        ...
        psi = self._front_sharpen(psi, dt)
