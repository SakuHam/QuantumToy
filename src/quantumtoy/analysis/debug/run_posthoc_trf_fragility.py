...
OUT_ROOT = ROOT_DIR / "sweep_runs" / "posthoc_trf_fragility_sigmaT"
...
EXPERIMENT_ENV: dict[str, str] = {
    # Optional fixed overrides for all fragility runs.
}


DEFAULT_SIGMA_SCALES = [0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30, 1.50]
...
def build_skip_result(case: FragilityCase, existing_summary: dict[str, object]) -> dict[str, object]:
...
def case_dir_for(case: FragilityCase) -> Path:
    return OUT_ROOT / case.base_case_name / case.case_name


def case_prefix_for(case: FragilityCase) -> Path:
    return case_dir_for(case) / case.case_name


def case_summary_path(case: FragilityCase) -> Path:
    return case_dir_for(case) / f"{case.case_name}_summary.json"


def case_log_path(case: FragilityCase) -> Path:
...
def build_cases(
    ...
    cases: list[FragilityCase] = []

    for row in chosen_rows:
        base_case_name = str(row["case_name"])
        cfg = row.get("config")  # normally not present in summary.jsonl; kept just in case

        # Reconstruct seed from overrides if available, otherwise parse from case_name.
        base_seed = None
        ...
        if base_seed is None:
            try:
                base_seed = int(base_case_name.split("_")[-1].replace("p", ".").replace("m", "-"))
            except Exception:
                raise RuntimeError(f"Could not infer seed from case_name={base_case_name!r}")

        # Need sigma baseline from actual run config bundle meta, not sweep summary.
        base_npz = ROOT_DIR / "sweep_runs" / "posthoc_trf_seed_sweep" / base_case_name / f"{base_case_name}.npz"
        ...
        sigma_init = estimate_sigma_init_from_cfg(base_cfg_dict)

        for scale in sigma_scales:
            sigma_abs = float(scale) * sigma_init
            sigma_tag = safe_case_name(f"{scale:.2f}")
            case_name = f"{base_case_name}__sigscale_{sigma_tag}"

            case_overrides = {
                "DETECTOR_NOISE_SEED": str(base_seed),
                "CLICK_RNG_SEED": str(base_seed),
                "POSTHOC_TRF_SIGMAT": str(sigma_abs),
            }
...
def run_case(case: FragilityCase, python_bin: str, force_rerun: bool) -> dict[str, object]:
    start = time.time()
    ...
    header_lines = [
        ...
        f"[SIGMA SCALE] {case.sigma_scale}",
        f"[SIGMA ABS] {case.sigma_abs}",
        "[ENV OVERRIDES]",
    ]
    ...
    result = {
        ...
        "sigma_scale": float(case.sigma_scale),
        "sigma_abs": float(case.sigma_abs),
...
def rebuild_summary_jsonl_from_case_summaries(cases: list[FragilityCase], summary_jsonl: Path) -> int:
    count = 0
...
def parse_sigma_scales(text: str) -> list[float]:
    parts = [p.strip() for p in text.split(",") if p.strip()]
    vals = [float(p) for p in parts]
    if not vals:
        raise ValueError("No sigma scales given")
    return vals
...
def main() -> int:
    ...
    parser.add_argument(
        "--sigma-scales",
        type=str,
        default=",".join(f"{x:.2f}" for x in DEFAULT_SIGMA_SCALES),
        help="Comma-separated sigmaT scale factors, e.g. 0.5,0.7,1.0,1.3,1.5",
    )
    parser.add_argument(
        "--base-case-prefix",
        default=None,
    ...
    print("=" * 80)
    print("Posthoc TRF fragility sweep (sigmaT)")
    print("=" * 80)
    ...
    print(f"SIGMA_SCALES     : {sigma_scales}")
        ...
        for fut in as_completed(future_to_case):
            ...
            if result.get("skipped", False):
                ...
                print(
                    ...
                    f"scale={result.get('sigma_scale')} | "
            ...
            print(
                ...
                f"scale={result.get('sigma_scale'):.2f} | "
    ...
    print("")
    print("=" * 80)
    print("Fragility sweep complete")
    print("=" * 80)
